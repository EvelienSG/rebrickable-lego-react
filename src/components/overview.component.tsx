...
const Overview: React.FC = () => {
  ...
  const themesList = legoThemes.map(
  ...
  const setsList = themeSets.map((set: Set): ReactElement => {
    const favorite = favoriteSets.some(
      (favorite) => favorite.set_num === set.set_num
    );
    let favIcon;
    if (favorite) {
    ...
    return (
      <ListGroup.Item key={set.set_num} action>
        <Link to={`/details/${set.set_num}`}>
          {set.name}
          {favIcon}
        </Link>
      </ListGroup.Item>
